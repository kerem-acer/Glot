...
{
    /// <summary>Returns a sub-span starting at the given char offset.</summary>
    public LinkedTextUtf16Span Slice(int offset)
    {
        if (offset == 0)
        {
            return this;
    ...
    public LinkedTextUtf16Span Slice(int offset, int count)
    {
        if (count == 0)
        {
            return default;
        ...
        return new LinkedTextUtf16Span(_data!, startSeg, startIdx, endSeg, endIdx, count);
    }

#if NET6_0_OR_GREATER
    /// <summary>Slices by range.</summary>
    public LinkedTextUtf16Span this[Range range]
    {
        get
        {
    ...
    (int segment, int index) FindPosition(int charOffset)
    {
